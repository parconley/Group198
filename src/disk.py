...
class RegistrationError(Exception):
    """Raised when the register_disk exchange fails."""
...
def _validate_cli(ns: argparse.Namespace) -> None:
    name = ns.disk_name
    if not name.isalpha():
        raise ValueError("disk_name must contain only alphabetic characters")
    if len(name) == 0 or len(name) > 15:
        raise ValueError("disk_name length must be between 1 and 15 characters")
...
def _prepare_register_args(ns: argparse.Namespace) -> RegisterArgs:
    message_id = ns.message_id or _generate_message_id(ns.disk_name)
...
def _send_and_receive(args: RegisterArgs, payload: bytes) -> dict:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
        ...
        for attempt in range(1, attempts + 1):
            logging.info(
                "register_disk attempt %d/%d to %s:%d", attempt, attempts, *manager_addr
            )
            try:
                sock.sendto(payload, manager_addr)
            except OSError as exc:
                raise RegistrationError(f"failed to send register_disk: {exc}") from exc
            ...
            for line in (ln for ln in decoded.splitlines() if ln.strip()):
                ...
                if message.get("message_type") != "register_disk_response":
    ...
    raise RegistrationError("Did not receive a valid register_disk_response")
